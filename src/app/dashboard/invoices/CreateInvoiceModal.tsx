...
export function CreateInvoiceModal({ open, onOpenChange }: CreateInvoiceModalProps) {
  const [client,         setClient]         = useState('__none__')
      ...
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <div className="flex items-start justify-between px-6 py-5 border-b border-zinc-100 shrink-0">
            <div>
              <h2 className="text-base font-semibold text-black">Create New Invoice</h2>
              <p className="text-xs text-zinc-400 mt-0.5">
                Fill in the details below to draft or send.
              </p>
            </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={client} onValueChange={setClient}>
                    <SelectTrigger className="rounded-lg">
                      {client !== '__none__'
                        ? <span className="text-sm text-zinc-900">{client}</span>
                        : <span className="text-sm text-zinc-400">Select a client…</span>
                      }
                    </SelectTrigger>
                    <SelectContent>
                      {CLIENTS.map(c => (
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-zinc-100 shrink-0 bg-white">
            <DialogPrimitive.Close className="h-9 px-4 text-sm text-zinc-500 hover:text-black transition-colors duration-150 rounded-lg hover:bg-zinc-50">
              Cancel
            </DialogPrimitive.Close>
            <div className="flex items-center gap-2">
              <button className="h-9 px-4 text-sm font-medium text-zinc-700 bg-white border border-zinc-200 rounded-lg hover:border-zinc-300 hover:bg-zinc-50 transition-colors duration-150">
                Save Draft
              </button>
              <button className="h-9 px-5 text-sm font-medium text-white bg-black rounded-lg hover:bg-zinc-800 active:bg-zinc-900 transition-colors duration-150">
                Send Invoice
              </button>
            </div>
          </div>
